...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // NVFP4 KV cache extra coverage. The type_KV loop above only generates quantized
    // cases at hsk 64 and 72, so add an explicit hsk=128 nvfp4-nvfp4 case (the common
    // Llama-style head size). The mixed NVFP4/F16 K/V pairs only dispatch a kernel on
    // builds with the full quant matrix (-DGGML_CUDA_FA_ALL_QUANTS); on a standard build
    // they report "not supported", exactly like the q8_0/q4_0 mixed-type cases above.
